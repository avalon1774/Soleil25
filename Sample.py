...
class RIXSMap(BaseScan):
    ...
    def slice(self, absorption_energy: Optional[np.ndarray] = None, save = False):
        ...
        if absorption_energy is None:
            absorption_energy = [2469.8, 2471.1,2473.4, 2482.5] #values for bat 4
    ...
    def plot_RIXS(self, absorption_energy = None, vmax = None):
        "plot certain ROI f the RIXS map (default roi2) calibrated and optinally show lines where neergy was taken"
        if absorption_energy is None:
            absorption_energy = []
        ...
        for roi_id, roi in self.ROIs.items():
            ...
            fig, axs = plt.subplots(1, 1, figsize=(7, 7), squeeze=False)

            ax = axs[0, 0]
            ax.set_title(f"Sum ROI {roi[0]}-{roi[1]} of {filename}")
            im = ax.pcolormesh(E2, energy, pilatus_sum, shading='auto', vmax=vmax, cmap = 'managua' )
            ax.set_xlabel('Emitted Energy (eV)')
            ax.set_ylabel('Incident Energy (eV)')
            #fig.colorbar(im, ax=ax)
            ...
            for line in absorption_energy:
                ax.axhline(line, color = 'black', alpha = 0.5, linewidth = 1)
            fig.tight_layout()
    ...
    def segment_vertical_strips_cleaned(
            self,
            strip_width_px: int = 10,
            step_px: int | None = None,  # default: non-overlapping
            reduce: str = "sum",  # "sum" or "mean" (used if integrate_eV=False)
            integrate_eV: bool = False,  # integrate along emitted-energy axis in eV (needs calibration)
            include_partial: bool = True,  # keep last narrow strip
            peak_halfwidth_px: int = 10,  # half-width around peak to replace with background
            omit_last_px: int = 15,  # drop last N pixels (like in project_XAS)
            save: bool = True,
            kind: str = "pilatus_xas_strips_clean"  # used by get_spectrum_path()
    ):
        """
        Clean the RIXS map by replacing elastic peaks with their local linear background,
        then segment along emitted-energy (vertical) axis into strips and export CSV.

        Output CSV columns (per ROI):
            incident_energy, strip_000-010_(Ecenter eV), strip_010-020_(...), ...

        Notes
        -----
        - Cleaning uses calibration_data[roi_id]['gaussians'] with columns:
          ['bkg_c0', 'bkg_c1', 'g_center'].
        - Cleaning follows project_XAS: replace a window around each peak with
          y = c0 + c1 * pixel, then optionally drops the last `omit_last_px` columns.
        """
        if strip_width_px < 1:
            raise ValueError("strip_width_px must be >= 1")
        if step_px is None:
            step_px = strip_width_px
        if reduce not in {"sum", "mean"}:
            raise ValueError("reduce must be 'sum' or 'mean'")

        images = self.data["images"]  # (nE_incident, H, W)
        energies_incident = self.data["energies"]  # (nE_incident,)

        results_by_roi = {}

        for roi_id, (left, right) in self.ROIs.items():
            if roi_id not in self.calibration_data:
                raise ValueError(f"No calibration data found for ROI {roi_id}")

            # Sum over ROI (x) -> (nE_incident, H~195), H is emitted-energy pixel axis
            rixs_map = np.sum(images[:, :, left:right], axis=2).copy()
            nE, npx = rixs_map.shape

            # Calibration: E2(px) = k*px + n
            k, n = self.calibration_data[roi_id]["line"]
            px_axis = np.arange(npx, dtype=float)
            E2 = k * px_axis + n

            # ----- CLEAN: replace elastic peak neighborhoods by linear background -----
            gaussians = self.calibration_data[roi_id]['gaussians']  # DataFrame from energy_calibration
            for row_idx, row in gaussians.iterrows():
                if not (0 <= row_idx < nE):
                    continue  # safety if gaussians is shorter/offset

                peak = int(round(row['g_center']))
                c0 = float(row['bkg_c0'])
                c1 = float(row['bkg_c1'])

                start = max(0, peak - peak_halfwidth_px)
                end = min(npx, peak + peak_halfwidth_px)
                if start < end:
                    bg = c0 + c1 * px_axis[start:end]  # linear background
                    rixs_map[row_idx, start:end] = bg

            # Optionally drop the last N pixels (like project_XAS)
            if omit_last_px > 0 and omit_last_px < npx:
                rixs_map = rixs_map[:, :npx - omit_last_px]
                E2 = E2[:rixs_map.shape[1]]
            npx = rixs_map.shape[1]

            # ----- Build strip ranges along emitted-energy (pixel) axis -----
            strip_ranges = []
            if include_partial:
                s = 0
                while s < npx:
                    e = min(s + strip_width_px, npx)
                    if e > s:
                        strip_ranges.append((s, e))
                    s += step_px
            else:
                for s in range(0, npx - strip_width_px + 1, step_px):
                    strip_ranges.append((s, s + strip_width_px))

            # ----- Reduce each strip per incident energy -----
            data = {"incident_energy": energies_incident}

            for s, e in strip_ranges:
                block = rixs_map[:, s:e]  # (nE, width)
                # label by px-range and eV center
                e_center = float(E2[s:e].mean()) if e > s else float(E2[s])
                col_name = f"strip_{s:03d}-{e:03d}_({e_center:.2f}eV)"

                if integrate_eV:
                    vals = np.array([np.trapezoid(block[i], E2[s:e]) for i in range(block.shape[0])])
                else:
                    vals = block.sum(axis=1) if reduce == "sum" else block.mean(axis=1)
            ...
            # ----- Save per-ROI CSV -----
            if save:
                base = get_spectrum_path(self, roi_id, kind=kind)  # e.g. ".../{kind}_{roi}.csv"
                suffix = (
                    f"_w{strip_width_px}px"
                    f"_step{step_px}"
                    f"{'_int' if integrate_eV else f'_{reduce}'}"
                    f"_half{peak_halfwidth_px}px"
                    f"_drop{omit_last_px}px"
                )
                path = base.with_name(base.stem + suffix + base.suffix)
                path.parent.mkdir(parents=True, exist_ok=True)
                df.to_csv(path, index=False)
                logger.info(f"Saved CLEAN vertical strips for ROI {roi_id} to {path}")

            results_by_roi[roi_id] = df

        # stash for later use
        self.xas_strips_clean = results_by_roi
...
sample.scans[9].plot()
sample.scans[7].slice(save=True, absorption_energy = np.arange(2468.0,2480.2,0.4))
...
sample.scans[4].slice(save=True,absorption_energy = np.arange(2468.0,2480.2,0.4))
